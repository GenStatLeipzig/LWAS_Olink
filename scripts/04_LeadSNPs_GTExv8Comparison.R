...
load("../results/03_leadSNPs.RData")
leadpQTLs[,SNPGene:=paste(variant_id_hg38,gene,sep=":")]
...
dumTab1<-foreach (i=c(1:length(myeQTLs)))%do%{
  ...
  # Get lead pQTLs
  filt<-is.element(data1$SNPGene,leadSNPs$SNPGene)
  data2a<-data1[filt,]
...
leadpQTLs5 = leadpQTLs5[tissue != "Whole_Blood_LIFE",]
