...
#' ***
pQTLs_a = fread("../exampleData_Heart/pQTLs_combined_hg38.txt.gz")
pQTLs_f = fread("../exampleData_Heart/pQTLs_females_hg38.txt.gz")
pQTLs_m = fread("../exampleData_Heart/pQTLs_males_hg38.txt.gz")
...
#' There are 59 significantly associated genes in the combined setting, 47 in males only, and 48 in females only.
#' 
#' Hence, I can define the BBFDR as FDR * (92/k) in the different settings 
#' 
pQTLs_a[,BBFDR:= FDR * 92/59]
pQTLs_m[,BBFDR:= FDR * 92/47]
...
#' # Save ####
#' ***
save(myTab,file="../temp_Heart/02_OlinkGenes.RData")
write.table(myFDR_short,file="../results_Heart/02_pQTLs_sigGenes.txt",
            col.names = T,row.names = F, quote = F, sep="\t",dec = ",")

save(pQTLs_a,file="../results_Heart/02_pQTLs_all_cis.RData")
save(pQTLs_m,file="../results_Heart/02_pQTLs_males_cis.RData")
save(pQTLs_f,file="../results_Heart/02_pQTLs_females_cis.RData")
