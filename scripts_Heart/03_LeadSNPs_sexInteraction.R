...
#' 
load("../temp_Heart/02_OlinkGenes.RData")
load("../results_Heart/02_pQTLs_all_cis.RData")
load("../results_Heart/02_pQTLs_males_cis.RData")
load("../results_Heart/02_pQTLs_females_cis.RData")
...
table(duplicated(leadpQTLs1$gene))
table(leadpQTLs1$gene) # LPL & AGER

table(duplicated(leadpQTLs2$gene))
table(duplicated(leadpQTLs3$gene))
table(leadpQTLs3$gene) # LPL & IL17D
...
#' Okay, all SNPs are available in all settings, I simply delete the second entry per gene.
#' 
leadpQTLs1 = leadpQTLs1[!duplicated(gene)]
leadpQTLs3 = leadpQTLs3[!duplicated(gene)]
leadpQTLs<-rbind(leadpQTLs1,leadpQTLs2,leadpQTLs3)
leadpQTLs[,dumID:=paste(variant_id_hg38,gene,sep="__")]
...
#' ***
names(leadpQTLs_a)
myFDR4<-cbind(leadpQTLs_a[,c(24,19,18,2,1,12,7,9,13:15,17,11,22)],
              leadpQTLs_m[,c(7,9,13:15,17,11,22)],
              leadpQTLs_f[,c(7,9,13:15,17,11,22)])
names(myFDR4)
